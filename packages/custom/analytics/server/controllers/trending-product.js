...
module.exports.trending_product = function (req, res, next) {
  ...
  MongoClient.connect(url, function (err, db) {
    ...
    var report_query = [match, {$group: {"_id": "$sku", "sku": {"$first": "$sku"}, "count": {"$sum": 1}}}, {$project: {"_id": 0, sku: 1}}, {"$sort": {"count": -1}}, {"$limit": 15}];
    ...
    var find_from_product = function (results) {
      async.series([function (callback) {
        db.collection("product").aggregate([
          {"$match": {"sku": {$in: results}}},{"$group":{"_id":"$sku", "Name":{"$last":"$Name"}, "price":{"$last":"$price"}, "categories":{"$last":"$categories"}, "brand":{"$last":"$brand"}, "Description":{"$last":"$Description"}}},
